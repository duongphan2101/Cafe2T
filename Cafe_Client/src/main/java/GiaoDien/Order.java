...
public class Order extends JFrame implements ActionListener{
	...
	double TongTien;
	Client_ThucDon_dao tddao;
	Client_HoaDon_dao hddao;
	 ...
	 */
	public Order() throws UnknownHostException, ClassNotFoundException, IOException {
		...
		btn_dat.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				String cel = model.getValueAt(0, 0).toString();
				if(cel.isEmpty()) {
					...
					try {
						...
						lbl_tongTien.setText("Tổng Tiền: ");
							...
							Mau_HoaDon m = new Mau_HoaDon();
							m.setVisible(true);
							m.setHoaDon(maHDtrongCTHD);
		...
		jButton_1.setBounds(1050, 10, 100, 50);
		...
		lbltenql.setBounds(750, 6, 300, 80);
    ...
    private void loadData() throws UnknownHostException, IOException, ClassNotFoundException {
		...
		tddao = new Client_ThucDon_dao(socket);
		List<ThucDon> listTD = tddao.getaAlTD();
		draw(listTD, content);
	}
    ...
    public void loadThucDonDaChon(String maTD) throws UnknownHostException, IOException, ClassNotFoundException {
		Socket socket = new Socket("192.168.1.141", 6666);
		tddao = new Client_ThucDon_dao(socket);
	    DefaultTableModel model = (DefaultTableModel) table.getModel();
	...
	public void writeHoaDon(double tong) throws IOException, ClassNotFoundException {
		Socket socket = new Socket("192.168.1.141", 6666);
	...
	public void writeCTHD(String maHD) throws IOException {	
		 for (int i = 0; i < model.getRowCount(); i++) {
			 Socket socket = new Socket("192.168.1.141", 6666);
	         ...
	         HoaDon hd = new HoaDon(maHD, null, 0, 0, null);
	   		 ...
	   		 hddao = new Client_HoaDon_dao(socket);
	   		 hddao.insertChiTietHoaDon(cthd);
	     }
	}
    ...
    public String loadMa() throws UnknownHostException, IOException {
		Socket socket = new Socket("192.168.1.141", 6666);
		hddao = new Client_HoaDon_dao(socket);
    	String maHD = hddao.maNextHoaDon();
    	return maHD;
    }
    
    public String loadMaNV() throws UnknownHostException, ClassNotFoundException, IOException {
		Socket socket = new Socket("192.168.1.141", 6666);
    	String maNV = name;
    	nvdao = new Client_NhanVien_dao(socket);
    	List<NhanVien> nvs = nvdao.getNVByTen(maNV);
    	for(NhanVien nv : nvs) {
    		Object t []= {nv.getMaNV()};
    		maNV = t[0] + "";
    	}
