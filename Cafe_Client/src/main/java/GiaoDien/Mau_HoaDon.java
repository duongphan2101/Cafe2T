...
public class Mau_HoaDon extends JFrame {
	...
	public void setHoaDon(String maHD) throws IOException, ClassNotFoundException {
    	Socket socket = new Socket("192.168.1.141", 6666);
	    hddao = new Client_HoaDon_dao(socket);
	    HoaDon hds = hddao.getHDbyID(maHD);
	    lbl_ngayLap.setText("Ngày Lập: " + hds.getNgayLap());
	    lbl_nvLap.setText("Nhân Viên lập hóa đơn: " + hds.getNhanVien().getTenNV());
	    lbl_tongTien.setText("Tổng Tiền: " + df.format(hds.getTongTien()));

	    List<ChiTietHoaDon> listcthd = hddao.getCHITIETbyID(maHD);
	    int labelY = 328;
	    int labelHeight = 30;
	    int labelSpacing = 20;

	    for (ChiTietHoaDon cthd : listcthd) {
	    	socket = new Socket("192.168.1.141", 6666);
	        tddao = new Client_ThucDon_dao(socket);
	        ThucDon tdon = tddao.getThucDonByID(cthd.getThucDon().getMaTD());

	        JLabel lbl_ten = new JLabel(tdon.getTenThucDon().toString());
	        ...
	        JLabel lbl_gia = new JLabel(df.format(tdon.getDonGia()));
	        ...
	        JLabel lbl_soLuong = new JLabel(cthd.getSoLuong() + "");
